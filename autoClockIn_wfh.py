...
def dump_device_info():
    ...
    显示设备信息
    """
    size_str = os.popen('.\\Tools\\platform-tools\\adb.exe shell wm size').read()
    device_str = os.popen('.\\Tools\\platform-tools\\adb.exe shell getprop ro.product.device').read()
    Phone_os_str = os.popen('.\\Tools\\platform-tools\\adb.exe shell getprop ro.build.version.release').read()
    density_str = os.popen('.\\Tools\\platform-tools\\adb.exe shell wm density').read()
    print("""******************************\nScreen: {size}\nDensity: {dpi}\nDevice: {device}\nPhone OS: {Phone_os}\nHost OS: {host_os}
Python: {python}\n******************************""".format(
              size=size_str.strip(),
...
def pull_screenshot(file_name):
    ...
    if 1 <= SCREENSHOT_WAY <= 3:
        process = subprocess.Popen(
            '.\\Tools\\platform-tools\\adb.exe shell screencap -p',
        ...
        os.system('.\\Tools\\platform-tools\\adb.exe shell screencap -p /sdcard/screenshot.png')
        os.system('.\\Tools\\platform-tools\\adb.exe pull /sdcard/screenshot.png {}'.format(file_name))
...
def check_screenshot():
    ...
    if os.path.isfile('screenshot.png'):
        try:
            ...
            pass
    if SCREENSHOT_WAY < 0:
        print("Don't support the current device")
        sys.exit()
    pull_screenshot('./screenshot.png')
    try:
        Image.open('./screenshot.png').load()
        print('Using method {} get screenshot' % SCREENSHOT_WAY)
    except Exception:
        SCREENSHOT_WAY -= 1
        check_screenshot()
...
class AutoClockInOut:
    ...
    def wakeup_input_passward(self):
        ...
        Phone.wakeup()
        time.sleep(1)
        self.run_log("Unclock screen....")
        Phone.swipe(130, 870, 563, 622)
        time.sleep(1)
    ...
    def get_screenshot(self, file_name):
        os.system('.\\Tools\\platform-tools\\adb.exe shell screencap -p /storage/emulated/0/{}'.format(file_name))
        os.system('.\\Tools\\platform-tools\\adb.exe pull /storage/emulated/0/{} .'.format(file_name))
